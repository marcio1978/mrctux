...
def interpretar_comando(texto):
    ...
    prompt = (
        ...
        "para tocar música = amarok --play\n"
        "temperatura = clima\n"
        "para a música = amarok --stop\n"
        "mostra data = date\n"
        "câmera um executa mas não parar o assitente = camera1.sh\n"
        "monitoramento = monitoramento.sh\n"
...
def executar_comando(comando):
    ...
    caminho_script = os.path.join(SCRIPTS_DIR, comando.split()[0])  # pega só o nome do script sem parâmetro

    # Se for script do diretório
    if os.path.isfile(caminho_script) and os.access(caminho_script, os.X_OK):
        try:
            # Se for câmera, executa em segundo plano pra não travar
            if "camera" in comando:
    ...
    # Caso não for script e sim comando do sistema
    else:
        try:
